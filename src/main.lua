...
while running do
    ...
    elseif option == 6 then
        -- Call the function to remove a city from favorites menu, passing the 'api' object
        weatherDataBase.removeCityFromFavoritesMenu(api)

    elseif option == 7 then
        print("End the program. Goodbye!")
        running = false  -- Set the variable to end the loop

    else
        print("Invalid option")
    end

    -- After handling the selected option, check if the program should continue running
    if running then
        print("Press Enter to continue...")
        io.read()  -- Wait for user input before showing the menu again
    end
end